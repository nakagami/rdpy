...
class ClientCoreData(CompositeType):
    ...
    def __init__(self, readLen = None):
        ...
        self.imeFileName = String("", readLen = CallableValue(64), optional = True, unicode=True)
        ...
        self.clientDigProductId = String("", readLen = CallableValue(64), optional = True, unicode = True)
...
def writeConferenceCreateRequest(userData):
    ...
    csCoreStream = Stream()
    csCoreStream.writeType(userData.CS_CORE)
    log.debug(f"writeConferenceCreateRequest() cs_core={type(userData.CS_CORE)}:{len(csCoreStream.getvalue())}:{binascii.hexlify(csCoreStream.getvalue())}")
